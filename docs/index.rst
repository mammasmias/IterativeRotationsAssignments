...
It solves the so-called Linear Assignment Problem (LAP). More precisely,
it solves the problem sometimes referred to as `bottleneck-LAP`.

By first solving the LAP, computing the distance between two atomic structures :math:`A` and
:math:`B` becomes trivial.
...
.. _eq-pb3:
.. math::
   P_B B = \mathbf{R} A + \mathbf{t} \qquad (1)
...
.. math::
   ...
   \argmin_{\bf R,t} \big\{ D({\bf R}A + {\bf t}, B) \big\} \qquad (2)
...
:math:`{\bf t}` are such that Eq. :ref:`(1) <eq-pb3>` is satisfied, i.e. when the best match is found.

The function :math:`D` in Eq. :ref:`(2) <eq-argmin>` is computed by CShDA.
...
Notice the similarity of Eq. :ref:`(4) <eq-sofi2>` to Eq. :ref:`(1) <eq-pb3>`: the structure :math:`B` is now
