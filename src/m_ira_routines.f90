...
contains
  ...
  subroutine set_orthonorm_bas(vec1, vec2, basis, fail)
    use ira_precision
    use m_sofi_tools, only: collinearity_thr
    implicit none
  ...
  subroutine svd_forcerot(nat1, typ1, coords1_in, &
       ...
       rmat, translate, ierr)
    use ira_precision
    use m_ira_tools, only: determinant3x3, svd
    implicit none
    integer(ip), intent(in) :: nat1
    integer(ip), dimension(nat1), intent(in) :: typ1
    real(rp), dimension(3, nat1), intent(in) :: coords1_in
  ...
  subroutine get_gamma_m(nat1, typ1_in, coords1_in, &
    ...
    use m_ira_tools, only: permute_real_2d, permute_int_1d
    implicit none
    integer(ip), intent(in) :: nat1
  ...
  subroutine ira_unify(nat1, typ1_in, coords1_in, candidate_1, &
    ...
    use sorting_module, only: sort
    use m_cshda, only: cshda
    use m_ira_tools, only: permute_real_2d, permute_int_1d, permute_real_2d_back, permute_int_1d_back
    use m_set_candidate, only: select_rc
    implicit none
    integer(ip), intent(in) :: nat1
    integer(ip), dimension(nat1), intent(in) :: typ1_in
    real(rp), dimension(3, nat1), intent(in) :: coords1_in
  ...
  subroutine ira_svd(nat1, typ1_in, coords1_in, &
    ...
    use m_set_candidate, only: set_candidates
    implicit none
    integer(ip), intent(in) :: nat1
    integer(ip), dimension(nat1), intent(in) :: typ1_in
    real(rp), dimension(3, nat1), intent(in) :: coords1_in
