...
contains
  ...
  subroutine sofi_get_symmops( nat, typ_in, coords_in, sym_thr, prescreen_ih, n_so, op_list, ierr )
    use ira_precision
    use m_sofi_tools, only: nmax, m_thr, construct_reflection
    use m_ira_error
    use sorting_module, only: sort
    use m_ira_routines, only: set_orthonorm_bas
#ifdef DEBUG
    use timer
#endif
    implicit none
    ...
    fail_beta = .true.
    do i = 1, nat
       if( norm2( coords(:,i)) < small_norm) cycle
       do j = 1, nat
          if( norm2( coords(:,j)) < small_norm) cycle
          !!
          !! call to IRA lib
    ...
    nn = 0
    kloop_: do k = 1, nat
       if( abs( d_i - norm2(coords(:,k))) > 1.1_rp*sym_thr ) cycle
       if( typ(k) /= ti ) cycle
       do l = 1, nat
          if( k==l) cycle
          if( abs( d_j - norm2(coords(:,l))) > 1.1*sym_thr ) cycle
          if( typ(l) /= tj ) cycle
          !!
          !! call to IRA lib
          !! (re)-set gamma
  ...
  subroutine sofi_get_perm( nat, typ, coords, nbas, bas_list, perm_list, dHausdorff_list )
    ...
    use m_ira_routines, only: svd_forcerot
    implicit none
    integer(ip),                   intent(in) :: nat
    integer(ip), dimension(nat),   intent(in) :: typ
    real(rp), dimension(3,nat),    intent(in) :: coords
  ...
  subroutine sofi_get_combos( nat, typ, coords, nbas, bas_list, ierr )
    use ira_precision
    use m_sofi_tools, only: m_thr, nmax
    implicit none
    integer(ip), intent(in) :: nat
    integer(ip), dimension(nat), intent(in) :: typ
    real(rp), dimension(3, nat), intent(in) :: coords
  ...
  subroutine try_sofi( theta, nat, typ_in, coords_in, sym_thr, dd, nbas, op_list, dh, m_thr, success, ierr )
    use ira_precision
    use m_sofi_tools, only: nmax, epsilon
    use m_cshda, only: cshda
    implicit none
    ...
    if( any(found == 0)) not_crazy = .false.
    ...
    if( dh < min(sym_thr*1e-3_rp, epsilon) ) then
       do_refine = .false.
       is_valid = .true.
    end if
  ...
  subroutine is_new_sofi( rmat, nbas, op_list, m_thr, is_new )
    ...
    use m_sofi_tools, only: nmax, matrix_distance
    implicit none
  ...
  subroutine sofi_get_pg( nbas, op_list, pg, n_prin_ax, prin_ax, verb, ierr )
    ...
    do i = 1, nbas
       if( op(i) == OP_INVERSION ) cycle
       if( op(i) == OP_IDENTITY ) cycle
       !!
       if( skip_ax(i) == 1 ) cycle
       ax = ax_list(:,i)
       ...
       do j = 1, nbas
          ...
          !!
          if( abs(dot_product(ax, ax_list(:,j))) > 0.999_rp ) then
             !! is same ax
    ...
    if( maxval(multip_ax, mask=(op == OP_PROP_ROT)) > 1 ) then
    ...
    do i = 1, nbas
       ...
       if( n_int(i) == cn_val .and. &
            op(i) == OP_PROP_ROT .and. &
            multip_ax(i) == cn_multip ) then
          ! write(*,"(a,1x, 3f9.4)") "ax is prin:", ax_list(:,i)
          !! store only unique
          isnew = .true.
          do j = 1, n_prin_ax
             if( dot_product(ax_list(:,i), prin_ax(:,j)) > 0.999_rp ) isnew = .false.
          end do
          if( .not. isnew ) cycle
    ...
    if( n_c2 > 0) dd = dd/n_c2
    ! write(*,*) 'n_c2',n_c2
    ! write(*,*) 'dd is:',dd
    if( dd < 0.01_rp .and. n_c2 > 0 ) c2_perp = .true.
    ...
    do i = 1, nbas
       if( op(i) /= OP_IMPROP_ROT) cycle
       if( n_int(i) /= 0) cycle
       dot = abs( dot_product(cn_ax, ax_list(:,i)))
       if( dot > 0.99_rp ) has_sigma_h = .true.
       ! write(*,*) i, dot
    end do
    ...
    do i = 1, nbas
       ...
       if( abs( cross ) > 0.99_rp ) n_in_plane = n_in_plane + 1
    end do
    ! write(*,*) 'n_in_plane',n_in_plane, n_c2
    ! if( n_in_plane == n_c2) has_sigma_v = .true.
    if( n_in_plane > 0) has_sigma_v = .true.
    ...
    if( has_sigma_v ) then
       ...
       lp:do i = 1, nbas
          ...
          if( dot < 0.01_rp) then
             ...
             do j = 1, nbas
                if( j == i ) cycle
                if( op(j) /= OP_PROP_ROT ) cycle
                if( n_int(j) /= 2) cycle

                dotj = abs( dot_product( ax_list(:,j), cn_ax))
                if( dotj < 0.01_rp) then
                   ! write(*,*) i, j, dot,dotj
                   ! write(*,*) ax_list(:,i)
                   ! write(*,*) ax_list(:,j)
...
100 continue
    ...
    elseif( pg_err == -1 )then
       write(pg,'(a,a1)') trim(pg),'-'
    elseif( pg_err == -9 ) then
  ...
  subroutine sofi_analmat( rmat, op, n, p, ax, angle, ierr )
    ...
    if( abs(det) > 1.0_rp + epsilon ) then
       write(*,*) "input matrix:"
       write(*,'(3f9.4)') rdum(1,:)
       write(*,'(3f9.4)') rdum(2,:)
       write(*,'(3f9.4)') rdum(3,:)
    ...
    ! if( cosangl > 1.0 .or. cosangl < -1.0) cosangl = real(nint(cosangl))
    if( abs(cosangl) > (1.0_rp + epsilon) ) then
       ...
       ierr = ERR_ACOS_ARG
       return
    elseif( abs(cosangl) > 1.0_rp ) then
    ...
    if( angle < epsilon ) n = 0
    if( p == 0 ) p = 1
    !!
    !! C0 = E :: rotation of angle 0 around any axis is identity
    if( op(1:1)==OP_PROP_ROT .and. angle < epsilon ) op=OP_IDENTITY
    !!
    !! S2 = I :: rotation 0.5 and reflection about any axis is inversion
    if( op(1:1)==OP_IMPROP_ROT .and. abs(angle-0.5_rp) < epsilon ) op=OP_INVERSION
  ...
  subroutine sofi_ext_Bfield( n_op, op_list, b_field )
    use ira_precision
    use m_sofi_tools, only: op_valid_ext_b
    implicit none
  ...
  subroutine sofi_construct_operation( op, axis, angle, matrix, ierr )
    use ira_precision
    use m_sofi_tools, only: construct_rotation, construct_reflection
    use m_ira_error
    implicit none
  ...
  subroutine sofi_unique_ax_angle( n_mat, mat_list, op_out, ax_out, angle_out, ierr )
    ...
    do i = 1, n_mat
       do j = i+1, n_mat
          ...
          !!
          !! same ax, same angle
          if( dotp > dotp_equal .and. &
               abs(angle_diff) < 1e-3_rp ) then
             !! ops are ambiguous
             write(*,*) i, j
          ...
          if( dotp < -dotp_equal  )then
             if( abs( angle_diff ) < 1e-3_rp .or. &
                  abs(angle_sum) < 1e-2_rp ) then
                !! ops are ambiguous
  ...
  subroutine sofi_mat_combos( n_in, mat_in, n_out, mat_out )
    ...
    use ira_precision
    use m_sofi_tools, only: nmax, matrix_distance, m_thr
    implicit none
  ...
  subroutine state_update( n_old, n_op, op_list, has_sigma, has_inversion, n_u_c3, u_c3, terminate )
    ...
    !! condition for Ih: has sigma or inversion, and number of unique c3 axes >= 3
    !!
    !! NOTE: The variables which store the state need to be allocated and initialised in the caller!
    use ira_precision
    use m_sofi_tools, only: OP_PROP_ROT, OP_IMPROP_ROT, OP_INVERSION
    implicit none
    ...
    do i = 1, n_new
       ...
       c3ax: if( op == OP_PROP_ROT .and. n >= 3) then
          !!
          if( n_u_c3 >= 5 ) exit c3ax
          unique = .true.
          do iax = 1, n_u_c3
             dotp = dot_product( ax, u_c3(:,iax))
             if( abs(dotp) > 0.99_rp ) unique = .false.
          end do
          !!
          if( unique ) then
  ...
  subroutine sofi_check_collinear( nat, coords, collinear, ax_o )
    ...
    use ira_precision
    use m_sofi_tools, only: collinearity_thr, ax_convention
    implicit none
    integer(ip),                intent(in) :: nat
    real(rp), dimension(3,nat), intent(in) :: coords
